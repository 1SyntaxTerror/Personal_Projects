    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (new_fd < 0) {
                ...
                // Primesc nickname la conectare
                int bytes = recv(new_fd, buffer, NICKNAME_SIZE - 1, 0);
                if (bytes <= 0) {
                    close(new_fd);
                } else {
                    buffer[bytes] = '\0';
                    clients[client_count].fd = new_fd;
                    strncpy(clients[client_count].nickname, buffer, NICKNAME_SIZE);
                    ...
                    printf("Client nou: %s (fd=%d)\n", clients[client_count-1].nickname, new_fd);
                }
            }
        }
